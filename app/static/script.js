...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Only add native-submit handling when WorkbenchExecutor is NOT active.
  // WorkbenchExecutor intercepts form submit with e.preventDefault() and
  // manages the button state itself, so this listener would conflict.
  if (form && runBtn) {
    form.addEventListener("submit", function (e) {
      // If WorkbenchExecutor has taken over (AJAX mode), skip — it handles state
      if (window._workbenchExecutorActive) return;
      saveQuery();
