...
(function (root, factory) {
  ...
  function WorkbenchExecutor(opts) {
    ...
    if (this._form) {
      ...
      this._form.addEventListener('submit', function (e) {
        // Let pagination submits (goToPage) pass through natively
        var pageInput = self._form.querySelector('[name="page"]');
        if (pageInput && parseInt(pageInput.value, 10) > 1) {
          // Native submit for pagination
          return;
        }
        e.preventDefault();
  ...
  WorkbenchExecutor.prototype.execute = function (query) {
    // Signal to script.js that AJAX mode is handling this
    window._workbenchExecutorActive = true;
